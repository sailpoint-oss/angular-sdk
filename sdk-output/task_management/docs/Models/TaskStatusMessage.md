---
id: task_management-model-task-status-message
title: TaskStatusMessage
pagination_label: TaskStatusMessage
sidebar_label: TaskStatusMessage
sidebar_class_name: angularsdk
keywords: ['angular', 'Angular', 'sdk', 'TaskStatusMessage', 'task_management']
slug: /tools/sdk/angular/task_management/models/task-status-message
tags: ['SDK', 'Software Development Kit', 'TaskStatusMessage', 'task_management']
---

# TaskStatusMessage

Import this model from the entry point of its package:

```typescript
import { TaskStatusMessage } from 'sailpoint-angular-sdk/task_management';
```

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**type** | `string` | Type of the message | [default to undefined]
**localizedText** | `LocalizedMessage` |  | [default to undefined]
**key** | `string` | Key of the message | [default to undefined]
**parameters** | `Array<TaskStatusMessageParametersInner>` | Message parameters for internationalization | [default to undefined]

