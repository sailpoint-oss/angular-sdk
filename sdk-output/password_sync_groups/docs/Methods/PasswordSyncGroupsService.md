---
id: password_sync_groups-method-password-sync-groups
title: PasswordSyncGroups
pagination_label: PasswordSyncGroups
sidebar_label: PasswordSyncGroups
sidebar_class_name: angularsdk
keywords: ['angular', 'Angular', 'sdk', 'PasswordSyncGroups', 'password_sync_groups']
slug: /tools/sdk/angular/password_sync_groups/methods/password-sync-groups
tags: ['SDK', 'Software Development Kit', 'PasswordSyncGroups', 'password_sync_groups']
---

# PasswordSyncGroupsService
...
The service takes one object that holds every parameter. Its type is `CreatePasswordSyncGroupV1RequestParams`.
...
export class ExampleComponent {
  ...
  createPasswordSyncGroupV1(): void {
    const passwordSyncGroup: PasswordSyncGroup = ; // 
    this.api.createPasswordSyncGroupV1({ passwordSyncGroup: passwordSyncGroup }).subscribe({
...
The service takes one object that holds every parameter. Its type is `DeletePasswordSyncGroupV1RequestParams`.
...
export class ExampleComponent {
  ...
  deletePasswordSyncGroupV1(): void {
    const id: string = ; // The ID of password sync group to delete.
    this.api.deletePasswordSyncGroupV1({ id: id }).subscribe({
      next: (result) => console.log(result),
      error: (error) => console.error(error),
    });
  }
...
The service takes one object that holds every parameter. Its type is `GetPasswordSyncGroupV1RequestParams`.
...
export class ExampleComponent {
  ...
  getPasswordSyncGroupV1(): void {
    const id: string = ; // The ID of password sync group to retrieve.
    this.api.getPasswordSyncGroupV1({ id: id }).subscribe({
      next: (result) => console.log(result),
      error: (error) => console.error(error),
    });
  }
...
The service takes one object that holds every parameter. Its type is `GetPasswordSyncGroupsV1RequestParams`.
...
`Observable<Array<PasswordSyncGroup>>`
...
export class ExampleComponent {
  ...
  getPasswordSyncGroupsV1(): void {
    ...
    this.api.getPasswordSyncGroupsV1({  }).subscribe({
      next: (result) => console.log(result),
      error: (error) => console.error(error),
    });
  }
...
The service takes one object that holds every parameter. Its type is `UpdatePasswordSyncGroupV1RequestParams`.
...
export class ExampleComponent {
  ...
  updatePasswordSyncGroupV1(): void {
    const id: string = ; // The ID of password sync group to update.
    const passwordSyncGroup: PasswordSyncGroup = ; // 
    this.api.updatePasswordSyncGroupV1({ id: id, passwordSyncGroup: passwordSyncGroup }).subscribe({
      next: (result) => console.log(result),
      error: (error) => console.error(error),
    });
  }
